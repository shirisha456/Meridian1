...
class JsonFormatter(logging.Formatter):
    """Structured JSON logs — one object per line, ready for Loki. See
    apps/core-api/app/core/logging.py for the same formatter there."""
...
def configure_logging(level: str, environment: str) -> None:
    ...
    # See apps/core-api/app/core/logging.py for why this checks isatty()
    # rather than `environment` alone — `docker compose up` also runs
    # with ENVIRONMENT=development, but its stdout goes to Promtail, not
    # a human, and needs the structured form.
